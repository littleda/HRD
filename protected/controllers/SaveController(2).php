...
//    }
    ...
    function actionInsertUpdate($id = NULL, $letDef = NULL, $No = NULL) {
        if (!empty($id) && !empty($letDef) && !empty($No)) {
            $model = EmpModels::model()->findByPk($id);
            $sql = "select * from training WHERE ID ='$id' and letterDef='$letDef' and No='$No'";
...
//-------------- check input value -------------
            $sql = "select ID,letterDef,No from training where ID='$model2->ID' and letterDef='$model2->letterDef'  and No='$model2->No'";
...
//                $drFind = EmpModels::model()->findBySql($sql);
                if (!empty($drFind)) {
                    if ($model2->ID == $drFind->ID && $model2->letterDef == $drFind->letterDef && $model2->No == $drFind->No) {
            ...
            if (!empty($letDef)) {
                if (!empty($drFind)) {
                   // echo $model2->No . "----" . $drFind->No . "----" . $No;
                    //exit(0);
                    if (($model2->letterDef != $letDef ) || ($model2->No != $No)) {
                        echo "<script type=\"text/javascript\"> alert(\"มีการบันทึกข้อมูลนี้แล้วนะจ๊ะ\");</script>";
                        $this->render('_form', array(
                            'model' => $model,
                            'model2' => $model2
...
//--------------The end check input value -------------
            ...
            if ($model2->save()) {
                // $this->redirect(array('View'));
                $this->redirect('index.php?r=save/View&id=' . $model2->ID);
            }
        }
    ...
    function actiondelete($id = NULL, $letDef = NULL, $No = NULL) {
        //$model2 = new TrainingModels();
        if (!empty($id)) {
            $sql = "SELECT * FROM training WHERE ID ='$id' and letterDef='$letDef' and No='$No'";
